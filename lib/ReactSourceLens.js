...
export function useReactSourceLens(options = {}) {
  ...
  useEffect(() => {
    ...
    const keyHandler = (e) => {
      try {
        // Detect platform (modern approach, avoiding deprecated navigator.platform)
        const isMac = (() => {
        ...
        // Toggle overlay with Cmd+Shift+L (Mac) or Ctrl+Shift+L (Windows/Linux)
        ...
        // Open source modal: Cmd+Shift+O (Mac) or Alt+Shift+O (Windows/Linux)
        const isOpenShortcut = e.shiftKey && e.code === "KeyO" && 
                              ((isMac && e.metaKey) || (!isMac && e.altKey));
